...
\only<1>{
    \Large{Let's take an example.}
}

\only<2>{
    \begin{figure}
        \centering
        \includegraphics[height=0.4\textwidth]{demo-1.png}
    \end{figure}
}
...
\only<4>{
    \begin{figure}
        \centering
        \includegraphics[width=\linewidth]{demo-3.png}
    \end{figure}
}

\only<5>{
    \large{This is a Maximum Leaf Spanning Tree problem.}
}
...
\begin{columns}
    \begin{column}{0.5\textwidth}
        \begin{itemize}[<+->]
            ...
            \item Case 1: $v\in V$
            %  by the definition of vertex cover we know that $v$ is dominated by $S$
...
% Rabid, start here 
\begin{frame}{Applications - \textsc{MaxLST}}
...
\only<2>{
    \begin{block}{Broadcasting Network}
...
\only<3>{
    \begin{figure}
        \centering
        \includegraphics[height=0.4\linewidth]{max-lst-1.png}
        \caption{Maximum number of leaves in a broadcasting network}
    \end{figure}
}

\only<4>{
    \begin{block}{Distribution Networks}
...
\only<5>{
    \begin{figure}
        \centering
        \includegraphics[width=\linewidth]{max-lst-2.png}
        \caption{A distribution network}
    \end{figure}
}
...
\only<1>{
    \begin{block}{Security of Private Systems}
...
\only<2>{
    \begin{figure}
        \centering
        \includegraphics[width=\linewidth]{min-lst-1.png}
        \caption{A private system with vulnerable endpoints}
    \end{figure}
}

\only<3>{
    \begin{block}{Database Indexing (Demers et al. 1998, Oracle Corp)}    
...
\only<4>{
    \begin{figure}
        \centering
        \includegraphics[width=\linewidth]{min-lst-2.png}
        \caption{Database indexing system using MinLST}
    \end{figure}
}
...
\begin{algorithm}[H]
    \KwData{A graph $G=(V,E), IN, BN, LN, FL \subseteq V$}
    apply all reduction rules possible\; \pause
    \uIf{there is some unreachable $v$ in $FL \cup free$}{\Return {$0$}} \pause 
    \uIf{$V = IN \cup BN$}{\Return {$|BN|$}} \pause 
    Take the vertex $v \in BN$ of the maximum degree\; \pause 
    \uIf{$deg(v) \geq 3$ or $(deg(v) == 2$\ and \ $N_{FL}(v) \neq \emptyset)$}
...
\begin{algorithm}[H]
    \uIf{$deg(v) == 2$}{ 
        Let $x_1$ and $x_2$ be two neighbors of $v$ such that $deg(x_1) > deg(x_2)$\;
        \If{$deg(x_1) == 2$}{
            Let $z = N(x_1) - v$\; 
            \If{$z \in Free$}{
                \Return{
...
\begin{algorithm}[H]
    ...
    \uIf{there is some unreachable $v$ in $FL \cup free$}{\Return {$0$}} 
    \uIf{$V = IN \cup BN$}{\Return {$|BN|$}} 
